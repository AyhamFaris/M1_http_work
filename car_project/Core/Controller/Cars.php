...
class Cars
{
    ...
    {
        try{
            if(!isset($request_body->name) && !isset($request_body->price)){
                $this->http_code = 422;
                throw new \Exception("name&price_not_found");
            }

            if (!$this->db->submit_query("INSERT INTO cars_info (name,price,model,description) VALUES ('{$this->request_body->name}','{$this->request_body->price}','{$this->request_body->model}','{$this->request_body->description}')")) {
                $this->http_code = 500;
                throw new \Exception('server_error');
            }
